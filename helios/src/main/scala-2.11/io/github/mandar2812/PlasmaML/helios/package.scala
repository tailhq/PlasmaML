...
package object helios {
  ...
  val gauss_standardization: DataPipe2[Tensor, Tensor, ((Tensor, Tensor), (MinMaxScalerTF, GaussianScalerTF))] =
  ...
  val scale_helios_dataset = DataPipe((dataset: HeliosDataSet) => {
    ...
    val (norm_tr_data, scalers) = gauss_standardization(dataset.trainData, dataset.trainLabels)
