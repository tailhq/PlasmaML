...
def main(
  ...
  val (net_layer_sizes, layer_shapes, layer_parameter_names, layer_datatypes) =
    timelag.get_ffnet_properties(d, num_pred_dims, num_neurons, "FLOAT32")

  val output_mapping = timelag.get_output_mapping(
...
def stage_wise(
                ...
                activation_func_i: Int => Activation = timelag.getReLUAct(1),
                activation_func_ii: Int => Activation = timelag.getReLUAct(10),
                iterations: Int                    = 150000,
                miniBatch: Int                     = 32,
                optimizer: Optimizer               = tf.train.AdaDelta(0.01),
                sum_dir_prefix: String             = "const_v",
  ...
  val dataset: timelag.TLDATA = timelag.generate_data(compute_output, d, n, noise, noiserot, alpha, sliding_window)

  val dataset_test: timelag.TLDATA = timelag.generate_data(compute_output, d, n, noise, noiserot, alpha, sliding_window)

  val (net_layer_sizes_i, layer_shapes_i, layer_parameter_names_i, layer_datatypes_i) =
    timelag.get_ffnet_properties(d, sliding_window, num_neurons_i, "FLOAT32")

  val (net_layer_sizes_ii, layer_shapes_ii, layer_parameter_names_ii, layer_datatypes_ii) =
    timelag.get_ffnet_properties(d, sliding_window, num_neurons_ii, "FLOAT32", net_layer_sizes_i.length - 1)
