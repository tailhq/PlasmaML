...
object Narmax extends DataPipe[DenseVector[Double], Double] {
  val narmax_params = DenseVector(0.8335, -3.083e-4, -6.608e-7, 0.13112,
    -2.1584e-10, 2.8405e-5, 1.5255e-10, 7.3573e-5, 0.73433, 1.545e-4)
...
object TestOmniNarmax {
  ...
  var (couplingFunc_t_1, couplingFunc_t_2, couplingFunc_t_3) =
  ...
  val processOmni = fileToStream >
    ...
    IterableDataPipe((couple: (Double, DenseVector[Double])) => {
  ...
  def apply(start: String, end: String) = {
    ...
    val processDates
      : DataPipe[(String, String), Iterable[(Double, DenseVector[Double])]] =
      DataPipe(
        ...
        DataPipe(
          (dates: (DateTime, DateTime)) =>
            "data/omni2_" + dates._1.getYear.toString + ".csv"
        ) >
        processOmni
    ...
    val mpoPipe = processDates > filterDataARX > IterableDataPipe(
      (couple: (Double, DenseVector[Double])) => {

        val finalFeatures = DenseVector(
          dst_t_1,
          couplingFunc_t_1,
          couplingFunc_t_1 * dst_t_1,
          dst_t_2,
          math.pow(couplingFunc_t_2, 2.0),
          couplingFunc_t_3,
          math.pow(couplingFunc_t_1, 2.0),
          couplingFunc_t_2,
          1.0,
          math.pow(dst_t_1, 2.0)
        )
        ...
        couple._1.toString + "," + couple
          ._2(0)
          .toString + "," + predicted.toString
      }
    ) >
      DataPipe[Iterable[String], Stream[String]](_.toStream) >
  ...
  def apply(start: Int, end: Int) = {
    ...
    for (year <- start to end) {
      ...
      streamToFile("data/NM_" + year + "_Res.csv").run(lines.toStream)
    }
  }
      ...
      fileToStream >
        ...
        IterableDataPipe((stormEventData: String) => {
          ...
          val NMFilePipe = fileToStream >
            IterableDataPipe((s: String) => s.split(",").map(_.toDouble)) >
            IterableDataPipe(
              (line: Array[Double]) =>
                line.head >= startStamp && line.head <= endStamp
            ) >
            IterableDataPipe((line: Array[Double]) => (line.last, line(1))) >
            DataPipe((s: Iterable[(Double, Double)]) => {
              val sc  = s.toList
              val met = new RegressionMetrics(sc.toList, sc.length)

              val minPredDst = sc.map(_._1).min
              val tp         = sc.map(_._1).indexOf(minPredDst)

              val minAct = sc.map(_._2).min
              val ta     = sc.map(_._2).indexOf(minAct)
  ...
  def apply(
    ...
    val preProcessPipe =
      fileToStream >
        ...
        IterableDataPipe((couple: (Double, DenseVector[Double])) => {
          ...
          //Calculate the coupling function p^0.5 V^4/3 Bt sin^6(theta)
          val Bt =
            math.sqrt(math.pow(features(2), 2) + math.pow(features(3), 2))
          val sin_theta6 = math.pow(features(2) / Bt, 6)
          val p          = features(4)
          val v          = features(1)
          val couplingFunc = math
            .sqrt(p) * math.pow(v, 4 / 3.0) * Bt * sin_theta6
          val Dst = features(0)
          (couple._1, DenseVector(Dst, couplingFunc))
        }) > IterableDataPipe(
        (couple: (Double, DenseVector[Double])) =>
        ...
        IterableDataPipe((couple: (DenseVector[Double], Double)) => {
          val vec     = couple._1
          val Dst_t_1 = vec(0)
          val Dst_t_2 = vec(1)
        ...
        }) > DataPipe((scoresAndLabels: Iterable[(Double, Double)]) => {

        val sc      = scoresAndLabels.toList
        val metrics = new RegressionMetrics(sc, sc.length)

        metrics.print()
        metrics.generatePlots()

        //Plotting time series prediction comparisons
        line((1 to sc.length).toList, sc.map(_._2))
        hold()
        line((1 to sc.length).toList, sc.map(_._1))
        legend(List("Time Series", "Predicted Time Series (one hour ahead)"))
        unhold()

        logger.info("Printing One Step Ahead (OSA) Performance Metrics")
        metrics.print()
        val (timeObs, timeModel, peakValuePred, peakValueAct) =
          names(40) match {
            case "Dst" =>
              (
    ...
    preProcessPipe.run("data/omni2_" + yearTest + ".csv").toSeq
  }
}
...
object TestOmniTL {
  ...
  def apply(
    ...
    val preprocessTLFile = fileToStream >
      ...
      DataPipe((tlData: Iterable[(Double, Double)]) => {
        tlData.grouped(6).toStream.map { gr =>
          (gr.head._1, gr.map(_._2).sum / gr.toSeq.length.toDouble)
        }
      })
    ...
    val tlDstPrediction = if (fileNameS == fileNameE) {
      val pipeTL = preprocessTLFile > IterableDataPipe(
        (couple: (Double, Double)) => couple._2
      )
      pipeTL.run("data/" + fileNameS)
    } else {
      val pipeTL = DataPipe(preprocessTLFile, preprocessTLFile) >
        DataPipe(
          (couple: (Stream[(Double, Double)], Stream[(Double, Double)])) => {
            couple._1 ++ couple._2
          }
        ) >
        IterableDataPipe((couple: (Double, Double)) => couple._2)
      pipeTL.run(("data/" + fileNameS, "data/" + fileNameE))
    }

    val scoresAndLabels = (tlDstPrediction zip actualDst).toList
      ...
      case "test" =>
        Seq(
          Seq(
            yearStart.toDouble,
            1.0,
            scoresAndLabels.length.toDouble,
            metrics.mae,
            metrics.rmse,
            metrics.Rsq,
            metrics.corr,
