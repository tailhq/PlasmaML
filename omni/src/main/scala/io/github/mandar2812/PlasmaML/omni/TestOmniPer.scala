...
object TestOmniPer {
  def apply(
    ...
    val extractTimeSeries = (lines: Iterable[String]) =>
      lines.map { line =>
    ...
    val deltaOperation = (lines: Iterable[(Double, Double)]) =>
      lines.toList
        .sliding(2)
        .map((history) => {
          val features =
            DenseVector(history.take(history.length - 1).map(_._2).toArray)
    ...
    val preProcessPipe = DataPipe(utils.textFileToStream _) >
      ...
      IterableDataPipe((line: String) => !line.contains(",,")) >
