...
object TestOmniAR {
  ...
  def apply(
    trainstart: String = "2008/01/01/00",
  ...
  def runExperiment(
    ...
    val modelTrainTest =
      (trainTest: (
        ...
        val training_data = trainTest._1._1.toSeq
        val model         = new GPNarModel(deltaT, kernel, noise, training_data)
        val num_training  = training_data.length
        ...
        if (opt.contains("validationStart") && opt.contains("validationEnd")) {
          ...
          model.processTargets = StreamDataPipe(
            (predictionCouple: (Double, Double)) =>
              (
          ...
          val standardizeValidationInstances = IterableDataPipe(
          ...
          model.validationSet_(
            (processValidation > standardizeValidationInstances)
              .run("data/omni2_" + yearTrain + ".csv")
              .toSeq
          )
        ...
        if (action != "energyLandscape") {
          ...
          val test_data = trainTest._1._2.toSeq
          val res       = model.test(test_data)
          ...
          val scoresAndLabelsPipe =
            DataPipe(
              (res: Seq[
                (DenseVector[Double], Double, Double, Double, Double)
          ...
          if (stepPred > 0) {
            ...
            val predictedOutput: List[Double] = test_data
              .grouped(stepPred)
              .map((partition) => {
            ...
            val outputs = test_data.map(_._2).toList

            val res2             = predictedOutput zip outputs
            val scoresAndLabels2 = deNormalize1.run(res2)

            val mpoMetrics =
              new RegressionMetrics(scoresAndLabels2, scoresAndLabels2.length)

            logger.info(
              "Printing Model Predicted Output (MPO) Performance Metrics"
            )
            mpoMetrics.print()
            ...
            line(
              (1 to scoresAndLabels.length).toList,
              scoresAndLabels.map(_._2)
            )
            hold()
            line(
              (1 to scoresAndLabels.length).toList,
              scoresAndLabels.map(_._1)
            )
            line(
              (1 to scoresAndLabels2.length).toList,
              scoresAndLabels2.map(_._1)
            )
            legend(
              List(
                "Time Series",
                "Predicted Time Series (one hour ahead)",
            ...
            mpoRes = Seq(
              ...
              deltaT.toDouble,
              stepPred.toDouble,
              num_training.toDouble,
              test_data.length.toDouble,
              mpoMetrics.mae,
              mpoMetrics.rmse,
              mpoMetrics.Rsq,
              mpoMetrics.corr,
              mpoMetrics.modelYield,
          ...
          spline(
            (1 to scoresAndLabels.length).toList,
            scoresAndLabels.map(_._3)
          )
          hold()
          spline(
            (1 to scoresAndLabels.length).toList,
            scoresAndLabels.map(_._4)
          )
          legend(
            List(
              name1,
              "Predicted " + name1 + " (one hour ahead)",
              "Lower Bar",
              "Higher Bar"
            )
          )
          ...
          action match {
            ...
            case "predict_error_bars" =>
              scoresAndLabels.map(i => Seq(i._2, i._1, i._3, i._4))
          }

        } else {
          gs.getEnergyLandscape(startConf, opt)
            .map(k => {
              Seq(k._1) ++ kernel.blocked_hyper_parameters
                .map(p => kernel.state(p)) ++
                kernel.hyper_parameters
                  .filter(l => !kernel.blocked_hyper_parameters.contains(l))
                  .map(k._2(_)) ++
    ...
    trainTestPipe.run(
      ("data/omni2_" + yearTrain + ".csv", "data/omni2_" + yearTest + ".csv")
    )
...
object DstARExperiment {
  ...
  def apply(
    years: List[Int] = (2007 to 2014).toList,
      ...
      val stormsPipe =
        DynaMLPipe.fileToStream >
          ...
          IterableDataPipe((stormEventData: String) => {
            ...
            val res = TestOmniAR.runExperiment(
              trainstart,
              trainend,
              startDate + "/" + startHour,
              endDate + "/" + endHour,
