...
val dll_beta = 10d
  ...
  val (q_alpha, q_beta, q_gamma, q_b) = (0d, 0d, 1.5d, 0.35)
  ...
  val Q = (l: Double, t: Double) => (math.exp(q_alpha)*math.pow(l, q_beta) + q_gamma)*math.pow(10, q_b*Kp(t))
  ...
  val impulse_data: Stream[(Double, Double)] = {
  ...
  val blocked_hyp = {
    model.blocked_hyper_parameters ++
      model.hyper_parameters.filter(
        c => c.contains("dll") || c.contains("base::") || c.contains("tau_")
      )
  }
  ...
  val post_vecs = samples.map(c => DenseVector(c("Q_alpha"), c("Q_beta"), c("Q_gamma"), c("Q_b")))
  ...
  //Map("tau_alpha" -> 0x03B1.toChar, "tau_beta" -> 0x03B1.toChar, "tau_b" -> 'b')
  ...
  hold()
  histogram((1 to num_post_samples).map(_ => hyper_prior("Q_b").draw), 100)
